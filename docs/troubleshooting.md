...
1. 在主侧栏或“设置 → 划词与快捷键”确认“划词翻译”已开启；开关会立即保存。
...
快捷键可能被 Windows、VS Code 或其他应用占用。在“设置 → 划词与快捷键”点击“恢复推荐组合”，或点击对应快捷键后直接按下新组合键。注册失败时界面会恢复原值，且不会写入无效配置；若仍失败，完全退出冲突应用后重试。

## 主窗口没打开时快捷键无效

全局快捷键需要 LexiFlow 进程在后台运行。请在“设置 → 常规”开启“Windows 登录时启动”，并把“关闭主窗口时”设为“隐藏到托盘”。关闭主窗口后托盘仍存在，快捷键继续生效；从托盘选择“退出”会结束进程，此时 Windows 无法把快捷键事件发送给 LexiFlow。

## 朗读按钮不可用或没有声音

LexiFlow 使用 Windows 已安装的系统语音，不调用翻译模型。按钮提示“当前系统未安装匹配的语音”时，请在 Windows 语言/语音设置中安装对应语言的语音包后重启应用。中英文语音分别匹配翻译结果中的语言；缺少语音包不会影响翻译和单词本。

## 单词本无法保存

单词本写入 `%APPDATA%/LexiFlow/vocabulary.json`。检查目录写权限和磁盘空间；损坏的 JSON 会被隔离为带时间戳的 `.corrupt.*.json`，应用随后创建空单词本。不要把该文件内容附在公开问题中，因为其中可能包含用户笔记和上下文。
