...
async function toggleSelection(): Promise<void> {
  ...
  try {
    const result = await translator.settings.patch({
      type: "update-shortcuts",
      value: { enableSelectionTranslation: !previous }
    });
    settings.value = result.snapshot.settings;
    window.dispatchEvent(new CustomEvent("lexiflow:settings-updated", { detail: result.snapshot.settings }));
  } catch {
    settings.value.shortcuts.enableSelectionTranslation = previous;
  } finally {
