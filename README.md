...
- `npm run build` 会执行构建契约检查，模块格式或资源路径错误会直接失败。
- 普通 TypeScript 使用原生 TypeScript 7；Vue SFC 暂由 `@typescript/typescript6` 提供兼容检查，桥接集中在 `scripts/vue-typecheck.cjs`。

## 首次使用

1. 启动 Ollama，并确保已拉取模型，例如 `ollama pull qwen3:4b`。
2. 打开托盘菜单中的“设置”，确认地址和模型名称，点击“保存并测试连接”。
3. 在其他 Windows 应用中选中文字，按 `Alt + Space` 快速翻译。
4. 若快捷键冲突，在设置中改为 `Ctrl + Alt + T` 等组合。
...
更多问题见 [故障排查](docs/troubleshooting.md) 和 [架构说明](docs/architecture.md)。
